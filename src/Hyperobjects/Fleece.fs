...
module Fleece =
        ...
        static member FromJSON (_: DateTime) =
            ...
            | JString s ->
                if s = null
                    ...
                    else match DateTime.TryParseExact(s, [|"yyyy-MM-ddTHH:mm:ss.fffZ"; "yyyy-MM-ddTHH:mm:ssZ"; "o" |], null, DateTimeStyles.RoundtripKind) with
                         | true, t -> Success t
    ...
    // This is the Relay customization that was necessitated forking this project
    ...
    type FromJSONClass with
        static member FromJSON(_ : JsonValue) = function
          | JObject _ as j -> Success j
          | a -> failparse "JObject" a
        static member FromJSON(_ : JsonValue option) = function
          | JObject _ as j -> Success (Some j)
          | _ -> Success None
        static member FromJSON(_ : TimeSpan) = function
          | JString s ->
            match TimeSpan.TryParse(s) with
            | (true, t) -> Success t
            | _ -> failparse "TimeSpan" s
          | a -> failparse "JString" a
        static member inline FromJson (_:'a[]) = function
            | JsonValue.Array xs ->           
                let xs : ParseResult<'a[]> = xs |> traverseA fromJSON
                xs
            | a -> failparse "array" a
        ...
        static member inline FromJson (_:Set<'a>) = function
            | JsonValue.Array xs ->           
                ...
                xs |> map Set.ofArray
            | a -> failparse "list" a
          (* TODO this causes a whole bunch of stuff to static resolve to System.Uri... haven't a clue why
             not having this means having to
        static member FromJSON(_ : System.Uri) = function
          | JString s ->
            if System.Uri.IsWellFormedUriString(s, System.UriKind.Absolute) then Success (System.Uri(s))
            else failparse "Uri" s
          | a -> failparse "Uri" a
          *)
